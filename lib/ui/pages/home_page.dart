/// XiaoXin APP - 主页
/// 原生语音对话界面，集成 SessionManager 和对话展示
library;
...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  Future<void> _initialize() async {
    // SessionManager 已在 main.dart 初始化完成
    // 这里只设置回调
    ...
    // 启动常驻热词检测（原生对话界面启动时自动启用）
    final hasPermission = SessionManager.instance.hasPermission;
    debugPrint('🏠 HomePage: hasPermission=$hasPermission, starting KWS...');
    if (hasPermission) {
      await SessionManager.instance.startKws();
      debugPrint('🏠 HomePage: KWS started');
    } else {
      debugPrint('🏠 HomePage: No permission, skipping KWS');
    }
  }
